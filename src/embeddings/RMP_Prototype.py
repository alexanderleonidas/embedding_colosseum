...
class RMPEmbedding:
    ...
    def _rmp_projection(self, x: torch.Tensor):
        ...
        x_proj = torch.sign(x) * torch.abs(x) ** self.alpha  # applying the nonlinear scaling x^alpha taking into account the sign

        if x_proj.dim() == 2:   # normalization to the unit length, with regard to batching
